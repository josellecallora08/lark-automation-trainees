            ...
            for index, row in df.iterrows():
                try:
                    ...
                    fields_to_add = {
                        ...
                        "parent_record_id": payload['record_id'],  # Convert to string if required
                        "file": payload["file"][0]['file_token'],  # Use single string instead of list
                        "trainer": payload["uploaded_by"][0]["id"],  # Use single string instead of list
                        "user_rating": int(user_rating) if user_rating is not None else 0  # Convert to integer
                    }
