...
class EvaluateTrainees(CallbackHandler):
    ...
    async def handler(self, payload: Dict[str, str]):
        ...
        try:
            ...
            try:
                # Debugging: Fetch and log the table schema if available
                table_schema = self.ctx.base_manager.get_table_schema(
                    table_id=os.getenv('PROCESSED_TABLE_ID')
                )
                self.ctx.logger.info(f"Table schema: {table_schema}")
