...
class EvaluateTrainees(CallbackHandler):
    ...
    async def handler(self, payload: Dict[str, str]):
        ...
        try:
            ...
            # Add a static row with Jeff Mathew's name
            static_row = {
                "date": datetime.now(timezone.utc).strftime('%Y-%m-%d'),
                "username": "jeff.mathew",
                "email": "jeff.mathew@example.com",
                "status": "Static",  # Ensure this matches the allowed values in the table schema
                "name": "Jeff Mathew",
                "final_score": 100,  # Ensure this is within the allowed range
                "engagement_score": 30,  # Ensure this is within the allowed range
                "knowledge_score": 40,  # Ensure this is within the allowed range
                "critical_thinking_score": 30,  # Ensure this is within the allowed range
                "minigame_score": 10,  # Ensure this is within the allowed range
                "user_rating": 5.0  # Ensure this is within the allowed range
            }
            self.ctx.logger.info(f"Adding static row: {static_row}")
            ...
            try:
                ...
                self.ctx.logger.info(f"Static row added successfully: {response}")
            except Exception as static_row_err:
                self.ctx.logger.error(f"❌ Error adding static row: {static_row_err}")
