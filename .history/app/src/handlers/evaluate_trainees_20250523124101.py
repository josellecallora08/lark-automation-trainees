    ...
    async def handler(self, payload: Dict[str, str]):
        ...
        try:
            ...
            try:
                ...
                # Test each field individually to isolate the problematic field
                for field, value in static_row.items():
                    test_row = {field: value}
                    try:
                        self.ctx.logger.info(f"Testing field: {field} with value: {value}")
                        response = self.ctx.base_manager.create_record(
                            table_id=os.getenv('PROCESSED_TABLE_ID'),
                            fields=test_row
                        )
                        self.ctx.logger.info(f"Field {field} added successfully: {response}")
                    except Exception as field_err:
                        self.ctx.logger.error(f"❌ Error adding field {field}: {field_err}")
                        if hasattr(field_err, 'response'):
                            self.ctx.logger.error(f"API Response for field {field}: {field_err.response.text}")

                # Attempt to create the full record
                response = self.ctx.base_manager.create_record(
                    table_id=os.getenv('PROCESSED_TABLE_ID'),
                    fields=static_row
                )
