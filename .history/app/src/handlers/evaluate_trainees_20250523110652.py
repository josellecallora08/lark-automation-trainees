            ...
            for index, row in df.iterrows():
                try:
                    ...
                    fields_to_add = {
                        ...
                        "parent_record_id": str(payload['record_id']),  # Convert to string
                        "file": str(payload["file"][0]['file_token']),  # Ensure string
                        "trainer": str(payload["uploaded_by"][0]["id"]),  # Ensure string
                        "user_rating": float(user_rating) if user_rating is not None else 0.0  # Ensure float
                    }
